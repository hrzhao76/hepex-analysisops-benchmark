...
def test_hzz_l2_private_rubric_flags_all_zero_contract_shaped_outputs(tmp_path):
    root = Path(__file__).parent.parent
    task_dir = root / "tasks_public" / "t006_hzz4l_l2"
    rubric = yaml.safe_load((root / "private_rubrics" / "hzz4l_l2_private_rubric.yaml").read_text(encoding="utf-8"))
    write_json(tmp_path / "input_manifest.json", {"files": [{"path": "file_0.root"}]})
    write_json(
        tmp_path / "four_lepton_mass_spectrum.json",
        {
            "observable": "m4l",
            "bin_edges": [120.0, 125.0, 130.0],
            "data_counts": [0.0, 0.0],
            "total_background_counts": [0.0, 0.0],
            "total_background_uncertainty": [0.0, 0.0],
            "signal_counts": [0.0, 0.0],
        },
    )
    write_json(
        tmp_path / "four_lepton_excess_summary.json",
        {
            "method_type": "counting_window",
            "signal_region": [120.0, 130.0],
            "window_observed_yield": 0.0,
            "window_background_yield": 0.0,
            "window_signal_yield": 0.0,
            "window_numerator_yield": 0.0,
            "significance_proxy": 0.0,
            "significance_formula": "numerator_over_sqrt_background_plus_fractional_systematic",
        },
    )
    write_json(
        tmp_path / "submission_trace.json",
        {
            "workflow_stages": [
                {"stage_label": "manifest_ingest", "status": "completed"},
                {"stage_label": "four_lepton_selection", "status": "completed"},
            ...
            "scientific_decisions": [
                {
                    "decision": "returned empty outputs",
                    "reason": "placeholder",
                    "impact_on_analysis": "no valid analysis",
                }
            ],
            "input_samples_used": [],
    ...
    (tmp_path / "interpretation.md").write_text("No meaningful conclusion can be drawn.", encoding="utf-8")
    task = SimpleNamespace(
        id="t006_hzz4l_l2",
        type="hzz4l_l2",
        level="l2",
    ...
    report = score_submission(task, tmp_path, rubric, {"status": "ok", "hard_checks_passed": True})
    results = {check["id"]: check for check in report["check_results"]}

    assert not results["execution_evidence_is_positive_and_manifest_consistent"]["passed"]
    assert not results["mass_spectrum_series_are_nonzero"]["passed"]
    assert not results["background_spectrum_series_is_nonzero"]["passed"]
    assert not results["signal_spectrum_series_is_nonzero"]["passed"]
    assert not results["excess_summary_window_and_significance_are_consistent"]["passed"]
    assert not results["mc_normalization_scale_plausibility"]["passed"]
